...
String CityID = "cityid"; //Find your city ID at https://openweathermap.org/
int TimeZone = -8; //GMT -8
boolean b24hr = false; //24 Hour Time Toggle.
int updateTime = 1800; //Seconds
...
void loop() {
    ...
    if(counter >= updateTime) //Get new data when counter equals updateTime (Counter goes up by one per second)
    {
      counter = 0; //Reset Counter
      getWeatherData(); //Run Program
...
String convertGMTTimeToLocal(String timeS)
{
 ...
 }else
 {
  ...
  } else if (!b24hr && time >= 12 && time < 13) {
    timeS = String(time)+" PM";
  } else if (!b24hr && time >= 13) {
    timeS = String(time-12)+" PM";
  } else if (!b24hr) {
    timeS = String(time)+" AM";
  }
